...
class EKFSLAM:
    ...
    def Fu(self, x: np.ndarray, u: np.ndarray) -> np.ndarray:
        ...
        """
        Rot = utils.rotmat2d(x[2])
        
        Fu = la.block_diag(Rot, 1) # TODO, eq (11.14)
    ...
    def h(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m = m - x[0:2, None] - (rotmat2d(x[2]) @ self.sensor_offset)[:,None] # offset[:, None] # TODO, relative position of landmark to sensor on robot in world frame
        ...
        zpred_theta = [np.arctan2(m_n[1], m_n[0]) for m_n in zpredcart]
        """
    ...
    def H(self, eta: np.ndarray) -> np.ndarray:
        ...
        for i in range(numM):  # But this whole loop can be vectorized
            ...
            Hx[inds][0, :] = (zc[:,i].T/zr[i]) @ jac_z_cb                    # d/dx||z_b|| from assignment
            Hx[inds][1, :] = (zc[:,i].T @ Rpihalf.T) / (zr[i]**2) @ jac_z_cb # d/dx angle(z_b) from assignment
            Hm[inds, inds] = - Hx[inds, 0:2] # Equal to the negating of the first two cols of Hx
    ...
    def add_landmarks(
        ...
        for j in range(numLmk):
            ...
            lmnew[inds] = eta[:2] + zj[0] * rot[:, 0] + sensor_offset_world # TODO, calculate position of new landmark in world frame
    ...
    def update(
        ...
        if numLmk > 0:
            ...
            Rlarge = la.block_diag(*[self.R]*numLmk)
            
            
            S = H @ P @ H.T + Rlarge # TODO,
            assert (
            ...
            if za.shape[0] == 0:
                ...
                Rlarge = la.block_diag(*[self.R]*(len(za)//2))
                jo = -W @ Ha
